...
EXEC tempdb..[usp_AnalyzeSpaceCapacity] @DBs2Consider = 'ctx65extacc, IM_CR_BATCH'
...
BEGIN
	/*
		...
		Purpose:		This procedure can be used to generate automatic TSQL code for working with Data- Create and Restrict Database File Names' type.
	*/
...
/*	******************** Begin:	@getInfo = 1 *****************************/';
			...
			(
				SELECT	DB_ID, DB_Name, CASE WHEN d.is_read_only = 1 THEN 'Read_Only' ELSE DATABASEPROPERTYEX(DB_Name, 'Status') END as DB_State, [TotalFilesSize(GB)], FileGroup, STUFF(
								(SELECT ', '+ CHAR(10) + f2.FileSettings
								 FROM T_Files as f2
...
';
	...
	Import-Module "SQLDBATools.psm1"
